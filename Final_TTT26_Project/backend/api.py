...
class AnalysisRequest(BaseModel):
    ...
    run_cell_towers: bool = False   # changed from True so it doesn't run universally
...
def _score(results: dict, req: AnalysisRequest) -> dict:
    ...
    if comps == 0:
        ...
        # Dynamic sliding scale: peak score roughly around 10-15 comps,
        # degrading continuously as the local market saturates to avoid hard capping at 35.
        comp_score = max(15.0, min(100.0, 95.0 - (max(0, comps - 15) * 0.4)))
    ...
    weights = {
        ...
        "cell_coverage": req.weight_cell_coverage if (req.run_cell_towers or req.business_type == "telecom_towers") else 0.0,
...
OLA_MAPS_API_KEY = os.getenv(
    "OLA_MAPS_API_KEY",
    os.getenv("KRUTRIM_API_KEY",
              "q4YyJBEO6pok33P8dM261aNb8Jg1cRcn9T5KW9a4"),   # ← same key as no_hardware_backend.py
)
...
def compare_zones(req: CompareRequest):
    # Returns success status so the frontend UI can show the comparison complete toast.
    return {"status": "ok", "message": "Comparison complete"}
...
def analyze(req: AnalysisRequest):
    ...
    def _run_competitors():
        if _is_india(req.lat, req.lon) and OLA_MAPS_API_KEY:
            # Run Ola (full paginated fetch) and OSM in parallel for maximum coverage
            with ThreadPoolExecutor(max_workers=2) as _ex:
                f_ola = _ex.submit(test_competitors_ola, req.lat, req.lon, req.radius, business_type)
                f_osm = _ex.submit(test_competitors_osm, req.lat, req.lon, req.radius, business_type)
                comp_ola = f_ola.result() or {}
                comp_osm = f_osm.result() or {}

            # Merge and deduplicate — prefer Ola entry when place_id matches
            seen_merge: set = set()
            c_list: list    = []
            for c in comp_ola.get("competitors", []):
                pid = c.get("place_id") or ""
                key = pid or f"{c.get('name','')}:{round(c.get('lat',0),5)}:{round(c.get('lon',0),5)}"
                if key not in seen_merge:
                    seen_merge.add(key)
                    c_list.append(c)
            for c in comp_osm.get("competitors", []):
                key = f"{c.get('name','')}:{round(c.get('lat',0),5)}:{round(c.get('lon',0),5)}"
                if key not in seen_merge:
                    seen_merge.add(key)
                    c_list.append(c)

            e_ola      = comp_ola.get("elapsed_s", 0) or 0
            e_osm      = comp_osm.get("elapsed_s", 0) or 0
            named_list = [c.get("name") for c in c_list
                          if c.get("name") and c["name"].strip() != "Unnamed"]

            return {
                "total_count":   len(c_list),
                "named_count":   len(named_list),
                "named_list":    named_list,
                "locations":     [{"lat": c.get("lat"), "lon": c.get("lon"), "name": c.get("name")} for c in c_list if c.get("lat")],
                "competitors":   c_list,
                "coords_ok":     comp_ola.get("coords_ok", 0),
                "coords_failed": comp_ola.get("coords_failed", 0),
                "elapsed_s":     round(max(e_ola, e_osm), 2),
                "source":        "ola_maps_full + osm_overpass",
    ...
    if req.run_cell_towers or business_type == "telecom_towers":
    ...
    roads_blob = results.get("roads") or {}
    comp_blob = results.get("competitors") or {}
    pop_blob = results.get("population") or {}

    diagnostics = {
        "elapsed_s_total": elapsed_total,
        "tasks_completed": sorted(results.keys()),
        "tasks_failed": errors,
        "roads_km": roads_blob.get("total_length_km") or roads_blob.get("road_score_km"),
        "roads_ways": roads_blob.get("total_ways"),
        "competitors_total": comp_blob.get("total_count"),
        "competitors_source": comp_blob.get("source"),
        "competitors_named_list_len": len(comp_blob.get("named_list") or []),
        "population": pop_blob.get("population"),
        "population_source": pop_blob.get("source_detail"),
    }

    # Human-readable server log (uvicorn stdout)
    print("\n" + "═" * 60)
    print(f"  ANALYZE done in {elapsed_total}s  |  lat={req.lat:.5f} lon={req.lon:.5f}  r={req.radius}m")
    print(f"  Tasks OK: {', '.join(sorted(results.keys()))}")
    if errors:
        print(f"  Tasks ERR: {errors}")
    print(f"  Roads: {diagnostics['roads_km']} km, {diagnostics['roads_ways']} ways")
    print(f"  Competitors: {diagnostics['competitors_total']} ({diagnostics['competitors_source']}) "
          f"names={diagnostics['competitors_named_list_len']}")
    print(f"  Population: {diagnostics['population']} ({diagnostics['population_source']})")
    print("═" * 60 + "\n")
